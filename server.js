...
"7b. SOLO si dice que sí tiene experiencia: dile que le vas a hacer unas preguntas rápidas de limpieza y hazle estas, UNA POR MENSAJE, en este orden. Cada mensaje lleva UNA sola pregunta, nunca dos. " +
"(1a) Si una regadera tiene manchas blancas de agua (sarro) y jabón pegado, ¿qué productos usas para limpiarla? " +
"(1b) Cuando ya terminaste de limpiar la regadera, ¿qué haces al final? " +
...
"El aviso de 'Si no sabes el nombre del producto, no te preocupes, solo descríbemelo (el color, cómo es, para qué lo usas)' dilo UNA SOLA VEZ, en el mensaje donde anuncias las preguntas de limpieza (junto con la pregunta 1a). No lo repitas en ese mismo mensaje ni en las siguientes preguntas. Una buena descripción cuenta igual que el nombre correcto al calificar (ej: 'una esponja suave, no la de metal' es correcto para el horno). No le pidas fotos. Si la persona manda una foto, vas a ver un texto como '[La persona mandó 1 foto como respuesta...]': acéptala como su respuesta a la pregunta actual, dale las gracias por la foto en pocas palabras y sigue con la siguiente pregunta de la lista. No la califiques, no le pidas que lo explique con palabras y no comentes lo que hay en la foto (en el perfil pon 'mandó foto, revisar'). " +
...
"RESPUESTA MUY VAGA: si la respuesta no se puede calificar porque es muy vaga (ej: 'producto color azul', 'un líquido', 'lo normal'), hazle UNA sola pregunta de seguimiento corta y neutral (ej: '¿Y para qué sirve ese producto azul?' o '¿Cómo es ese líquido, para qué lo usas?'). Solo una vez por pregunta: conteste lo que conteste, pasa a la siguiente. No lo hagas si la respuesta ya se puede calificar, aunque sea incorrecta. " +
"Para calificar (esto es solo para ti, nunca lo digas): (1a) quita-sarro (CLR, Bar Keepers Friend, vinagre), fibra que no raye; (1b) enjuagar y SECAR al final; (2) limpiador de hornos o desengrasante, dejarlo actuar, y tallar con esponja o fibra que NO raye (mal: fibra de acero/brillo); (3) limpiador para acero inoxidable o trapo de microfibra, siguiendo la dirección de las líneas del metal; (4) de arriba hacia abajo: ventiladores/lámparas y lo de arriba primero, el piso al final; (5) en la taza del baño, nunca afuera ni en el jardín; (6) sí, trapos separados. VOCABULARIO: nunca uses la palabra 'inodoro'; di 'taza del baño'. " +
"Guarda en skill_notes un resumen muy corto de cada respuesta (ej: '1 no dijo secar, 2 ✓, 3 ✓...') y en skill_rating: strong, medium o weak. skill_notes va EN INGLÉS (ej: '1a ✓, 1b didn't say dry it, 2 ✓, 3 ✓...').\n" +
...
"\"profile\": {\"name\": null, \"city\": null, \"experience\": null, \"car\": null, \"supplies\": null, \"helper\": null, \"helper_name\": null, \"available_job\": null, \"accepted_pay\": null, \"meetup_ok\": null, \"interview_ok\": null, \"applied_before\": null, \"skill_rating\": null, \"skill_notes\": null, \"best_time\": null}, " +
"\"unknown_question\": null, \"questions_left\": null, " +
"\"status\": \"continue\" | \"call_ready\" | \"waitlist\" | \"not_interested\" | \"off_topic\"}\n" +
"questions_left = cuántas preguntas de la lista (pasos 2 al 11, incluyendo las de limpieza solo si tiene experiencia) quedan DESPUÉS de la pregunta que haces en este mensaje (número entero; 0 si esta es la última; null si este mensaje no hace una de esas preguntas, o si solo es una pregunta de seguimiento o una repetición). " +
"En helper_name pon quién es su ayudante si lo dice (ej: 'her mom', 'her sister Ana'). En profile llena solo lo que la persona ya dijo (texto corto o true/false), deja null lo demás. IMPORTANTE: todo lo que escribas en profile y en unknown_question va EN INGLÉS (es para el dueño, que lee en inglés); solo el reply va en español. " +
...
async function runScreening(state, text, now) {
    ...
    if (status === 'call_ready' && !state.alerts.callReady) {
        ...
        await sendAlert(state, (win.urgent ? '🚨 URGENT - ' : '') + 'CANDIDATE READY FOR A CALL', [
            ...
            'Car: ' + fmtBool(prof.car) + ' | Supplies: ' + fmtBool(prof.supplies) + ' | Helper: ' + fmtBool(prof.helper) + (prof.helper_name ? ' (' + String(prof.helper_name).slice(0, 40) + ')' : ''),
    ...
    q.timer = setTimeout(function() {
        ...
        withLock(from, async function() {
            ...
            var combined = (q.texts.length ? q.texts.join(' ') + ' ' : '') +
                '[La persona mandó ' + q.media.length + (q.media.length > 1 ? ' fotos' : ' foto') + ' como respuesta. Pete las revisa después.]';
